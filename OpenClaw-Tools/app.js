// data.mock.js 提供 mockCommandsDatabase / commandsDB 等
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.__openclawNavigate = navigateToPage;
    ...
    */
    ...
    function captureCommandsInlineDraft(mount) {
        if (!mount) return;
        const textarea = mount.querySelector('.command-inline-input-box');
        const sensitiveToggle = mount.querySelector('.command-inline-sensitive-toggle input');
        ...
        if (textarea) {
            commandsInlineDraftState.value = textarea.value || '';
            commandsInlineDraftState.selectionStart = textarea.selectionStart;
            commandsInlineDraftState.selectionEnd = textarea.selectionEnd;
            commandsInlineDraftState.focused = document.activeElement === textarea;
        } else {
            commandsInlineDraftState.focused = false;
        }
        commandsInlineDraftState.sensitive = Boolean(sensitiveToggle?.checked);
    ...
    function restoreCommandsInlineDraft(mount, session) {
        ...
        const sensitiveToggle = mount.querySelector('.command-inline-sensitive-toggle input');
        if (!textarea) return;
        textarea.value = commandsInlineDraftState.value || '';
        if (sensitiveToggle) {
            sensitiveToggle.checked = Boolean(commandsInlineDraftState.sensitive);
        }
        if (commandsInlineDraftState.focused) {
            textarea.focus();
            if (typeof commandsInlineDraftState.selectionStart === 'number' && typeof commandsInlineDraftState.selectionEnd === 'number') {
                textarea.setSelectionRange(commandsInlineDraftState.selectionStart, commandsInlineDraftState.selectionEnd);
            }
            return;
        }
    ...
    function syncCommandsInlineDraftFromMount(mount, sessionId) {
    ...
    function resetCommandsInlineDraft(sessionId) {
        ...
        commandsInlineDraftState.selectionStart = 0;
        commandsInlineDraftState.selectionEnd = 0;
        commandsInlineDraftState.focused = false;
        commandsInlineDraftState.surfaceFocused = false;
    }
    ...
    function buildCommandsInlineInteractiveHTML(session) {
        ...
        const statusText = session.status === 'running'
            ...
            : '\u4ea4\u4e92\u4f1a\u8bdd\u5df2\u7ed3\u675f\uff0c\u4e0d\u518d\u63a5\u53d7\u65b0\u8f93\u5165\u3002';
        return `<div class="command-inline-interactive-tip ${session.status === 'running' ? '' : 'is-disabled'}">${statusText}</div>`;
    }
    ...
    function sendInlineInteractiveRawText(sessionId, value, options = {}) {
        ...
        window.api.sendCommandInput(sessionId, textValue, options.submit === true);
    ...
    function bindCommandsInlineInteraction(mount, session) {
        ...
        outputSurface?.addEventListener('keydown', (event) => {
            const isPlainTextKey = !event.ctrlKey && !event.metaKey && !event.altKey && String(event.key || '').length === 1;
    ...
    // 挂载带有 Preload 和自动登录脚本的跨进程 Webview
    async function initWebviewSecured() {
        const chatView = document.getElementById('view-chat');
        let appPath = '';
        if (window.api && window.api.getAppPath) {
        ...
        if (appPath) {
            ...
            preloadAttr = `preload="file:///${safePath}/webview-preload.js"`;
        }
        ...
        chatView.innerHTML = `
            <div id="webviewError" style="display:none; flex-direction:column; align-items:center; justify-content:center; height:100%; color:#8b8b93; background-color:#1e1e24; border-radius:12px;">
                ...
                <p>当前配置的地址: <code id="webviewErrorUrl">http://127.0.0.1:18789</code> 连接失败</p>
                <div style="margin-top: 24px; padding: 16px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 8px;">
                    ...
                    <ul style="font-size: 0.85rem; text-align: left; list-style-type: disc; padding-left: 20px; line-height: 1.6;">
                        <li>您可能还没有启动网关。请前往“首页”点击执行 <b>openclaw gateway</b>。</li>
                        <li>您的网关端口可能不是默认的 18789。您可以执行命令指定正确的端口。</li>
                        <li>这并非系统白屏 Bug，而是因为内置的对话前端未在后台运行。</li>
                    </ul>
                </div>
            ...
            <webview id="chatWebview" src="http://127.0.0.1:18789" ${preloadAttr} style="display:flex; width:100%; height:100%; border:none; border-radius:12px; background-color:#1e1e24;"></webview>
        `;
        ...
        webview.addEventListener('did-fail-load', (e) => {
            // e.errorCode < 0 表示真实的加载失败 (如 -102 ERR_CONNECTION_REFUSED)
            if (e.isMainFrame && e.errorCode < 0) {
                webview.style.display = 'none';
                webviewError.style.display = 'flex';
                document.getElementById('webviewErrorUrl').innerText = e.validatedURL || webview.src;
            }
        });

        webview.addEventListener('did-finish-load', () => {
            webview.style.display = 'flex';
            webviewError.style.display = 'none';
        });

        // 监听来自 Webview IPC 的 / 斜杠呼唤
        ...
        if (window.api && window.api.getOpenClawToken) {
            webview.addEventListener('dom-ready', async () => {
                const token = await window.api.getOpenClawToken();
                if (token) {
                    // 安全转义：使用 JSON.stringify 防止 Token 中特殊字符导致 XSS 注入
                    const safeToken = JSON.stringify(token);
                    webview.executeJavaScript(`
                        (function() {
                            var _t = ${safeToken};
                            localStorage.setItem('openclaw_auth_token', _t);
                            localStorage.setItem('auth_token', _t);
                            localStorage.setItem('token', _t);
                            
                            var input = document.querySelector('input[type="password"]') || document.querySelector('input[type="text"]');
                    ...
                    `).catch(err => console.error("Webview Autologin Failed:", err));
                }
            });
        }
    }
    ...
    async function initWebviewSecured() {
        ...
        chatView.innerHTML = `
            ...
            <div id="webviewError" style="display:none; flex-direction:column; align-items:center; justify-content:center; height:100%; color:#8b8b93; background-color:#1e1e24; border-radius:12px;">
                <h2 style="color:#ff6188; margin-bottom: 16px;">🚫 无法连接到智能对话服务</h2>
                <p>当前配置的地址: <code id="webviewErrorUrl">${escapeAttr(displayChatUrl)}</code></p>
                <p id="webviewErrorDetail" style="margin-top:8px;color:#b8c0d4;font-size:13px;max-width:640px;text-align:center;line-height:1.7;">请确认首页显示网关在线，或者检查 Gateway Token 与端口配置。</p>
                <div style="margin-top: 24px; padding: 16px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 8px;">
                    <p style="font-size: 0.95rem; font-weight: 600; color: #e0e0e0; margin-bottom: 8px;">💡 诊断建议：</p>
                    <ul style="font-size: 0.85rem; text-align: left; list-style-type: disc; padding-left: 20px; line-height: 1.6;">
                        <li>先到“首页”确认网关状态是不是在线。</li>
                        <li>如果网关在线但仍然空白，通常是 Token 未注入成功或 Control UI 首次加载过早。</li>
    ...
    function renderPage() {
        ...
        document.querySelectorAll('.page-view').forEach(v => {
            if (v.id === 'view-chat' && currentPage !== 'chat' && chatWebviewInitialized) {
        ...
        const runDeferredRender = (pageName, task) => {
            scheduleUiFlush(() => {
        ...
        if (currentPage === 'dashboard') {
            ...
            runDeferredRender('dashboard', () => {
                if (shouldReuseDashboard && typeof window.__openclawResumeDashboard === 'function') {
                    window.__openclawResumeDashboard();
                    return;
                }
                renderDashboardHTMLV2(el);
            });
            ...
            runDeferredRender('commands', () => {
                ...
                } else if (currentCategoryTab !== '所有') {
                    tabFiltered = filteredCommands.filter(c => {
                        if (currentCategoryTab === '通用') return !c.tags.some(t => categories.includes(t)) || c.tags.includes('通用');
                        return c.tags.includes(currentCategoryTab);
                    });
                }
            ...
            el.style.display = 'block';
            runDeferredRender('models', () => renderModelsPageV2(el));
            ...
            el.style.display = 'block';
            runDeferredRender('memory', () => window.OpenClawPanelPages?.renderMemoryCenterPage?.(el));
            ...
            items: [
                {
                    ...
                    label: '自启',
                    summary: autoStartEnabled ? '已启用开机自启' : '已关闭开机自启',
                    actionId: nextAutoStartAction?.id || '',
                    title: nextAutoStartAction?.title || '开机自启',
    ...
    function renderDashboardHTMLV2(container) {
        ...
        const mainActionMeta = { start: { icon: '🚀', label: '启动', cls: 'db-action-start' }, stop: { icon: '⏹', label: '停止', cls: 'db-action-stop' }, restart: { icon: '🔄', label: '重启', cls: 'db-action-restart' } };
        const mainBtnsHtml = mainActions.map(a => {
            const m = mainActionMeta[a.id] || { icon: '⚙', label: a.title, cls: '' };
            return `<button class="dashboard-action-btn ${m.cls}" data-action="${escapeHtml(a.id)}" data-title="${escapeHtml(a.title)}" data-preview="${escapeHtml(a.previewCommand)}">${m.icon} ${m.label}</button>`;
        }).join('');
        ...
        const quickCards = [
            { type: 'autostart', icon: '⚡', label: '自启', enableAction: enableAutostart, disableAction: disableAutostart },
            { type: 'fix', icon: '🔧', label: '修复' },
            { type: 'nav', icon: '⚙', label: '配置', page: 'settings' },
            { type: 'nav', icon: '📜', label: '日志', page: 'logs' },
            { type: 'nav', icon: '📋', label: '任务', page: 'cron' },
            { type: 'nav', icon: '💬', label: '聊天', page: 'chat' }
        ];
            ...
            <div class="dashboard-shell">
                <div class="db-status-panel">
                    <div class="db-status-indicator">
                        ...
                        <div id="dbGatewayDetail" class="db-status-subtitle">通过网关端口监听状态检测中...</div>
                    </div>
            ...
            <div class="db-quick-grid">
                ...
                <div class="db-log-summary">
                    <div class="db-log-summary-copy">
                        ...
                        <span id="dbLogPreview" class="db-log-inline-preview">首页不自动加载实时日志，完整日志请前往日志查看。</span>
                    </div>
        ...
        dashboardLogPreview?.replaceChildren(document.createTextNode('首页默认只展示运行摘要，完整日志可按需展开。'));
            ...
            dashboardLogSummary.insertAdjacentHTML('afterend', `
                <div id="dbLogBody" class="db-log-body" style="display:none;">
                    ...
                    <section class="dashboard-helper-section">
                        <div class="dashboard-tip-desc">展开后会在当前首页显示更多实时日志；关闭实时日志时会保留最近一次摘要。</div>
        ...
        container.querySelector('#dbModelCount')?.replaceChildren(document.createTextNode('检测中'));
        container.querySelector('#dbChannelCount')?.replaceChildren(document.createTextNode('检测中'));
        dashboardLogPreview?.replaceChildren(document.createTextNode('首页默认只展示运行摘要，完整日志可以按需展开。'));
        dashboardOpenLogsButton?.replaceChildren(document.createTextNode('展开日志'));
        if (dashboardRealtimeLogLabel) dashboardRealtimeLogLabel.textContent = '实时日志';
        container.querySelector('#dbLogReconnect')?.replaceChildren(document.createTextNode('重新连接'));
        ...
        const dashboardLogEmptyText = `等待 ${dashboardLogFollowCommand} 输出...`;
        ...
        if (dashboardLogDesc) {
            dashboardLogDesc.innerHTML = `这里直接显示 <code>${dashboardLogFollowCommand}</code> 的实时输出，方便从首页盯住 OpenClaw 整体运行日志。`;
        }
        ...
        function applyDashboardRealtimeLogPreference(options = {}) {
            ...
            if (!dashboardRealtimeLogEnabled) {
                ...
                syncRealtimeLogPreview('未勾选实时日志时，首页不更新日志；可点击右侧按钮展开查看更多日志。');
                if (logContent) {
        ...
        function setGatewayIndicator(payload = {}) {
            const online = Boolean(payload.online);
            const statusText = payload.statusText || (online ? '在线' : '离线');
            const detailText = payload.detail || '通过网关端口监听状态检测当前 Gateway。';
            ...
            gatewayDetail.textContent = `${detailText}${suffix}`;
        ...
        function appendDashboardLogChunk(chunk) {
            ...
            parts[0] = `${dashboardLogState.trailing || ''}${parts[0]}`;
        ...
        async function refreshGatewayStatus(options = {}) {
            ...
            try {
                ...
                setGatewayIndicator({
                    online: false,
                    statusText: '离线',
                    detail: `状态检测失败: ${error?.message || error}`
                });
            } finally {
        ...
        // Fix card — openclaw doctor --fix
        const fixCard = container.querySelector('#dbFixCard');
        if (fixCard) {
            fixCard.addEventListener('click', () => {
                const fixCmd = commandsDB.find(c => c.code === 'openclaw doctor') || { id: 'doctor-fix', code: 'openclaw doctor --fix', name: '修复' };
        ...
        if (logToggleBtn && logBody) {
            const doToggle = () => {
                ...
                logToggleBtn.textContent = isExpanded ? '展开 ▼' : '收起 ▲';
                if (openLogsPage) {
    ...
    function askParameterAsync(paramName) {
        return new Promise(async (resolve) => {
            ...
            let dynamicModels = [];
            let dynamicProviders = []; // 用于存放提供的商列表
            
            // 是否是要求输入"Provider" (提供商)
            const isProviderParam = paramName.includes('提供商') && !paramName.includes('模型') && !paramName.toLowerCase().includes('model');
            // 是否是要求输入"Model" (模型名，可能带有提供商如 <提供商/模型名>)
            const isModelParam = paramName.includes('模型') || paramName.toLowerCase().includes('model') || (paramName.includes('提供商') && paramName.includes('/'));
            ...
            // Setup UI
            paramModalDesc.textContent = `请输入参数所需的值: 对应 ${paramName}`;
            ...
            const renderDropdown = (query) => {
                ...
                if (filtered.length > 0) {
                    ...
                    filtered.forEach((m, idx) => {
                        ...
                        // Split provider and model for nice UI (如果存在斜杠)
                        const parts = m.split('/');
        ...
        block.innerHTML = `
            ...
            ${sessionInfo.interactive ? `
                <div class="command-session-input-panel">
                    <div class="command-session-input-hint">交互式命令会停在这里等待输入。文本框默认“发送并回车”，方向键和纯回车用快捷按钮发送。</div>
                    ...
                    <div class="command-session-shortcuts">
                        <button class="command-input-btn" data-input-key="${escapeHtml(sessionInfo.id)}" data-key="up" type="button">↑</button>
                        <button class="command-input-btn" data-input-key="${escapeHtml(sessionInfo.id)}" data-key="down" type="button">↓</button>
    ...
    function summarizeDashboardActionResult(session, code) {
        ...
        if (!isSuccess) {
            ...
            appendTerminalLog('[首页动作] 管理命令未正常完成，请结合上方输出继续排查。\n', 'sys', session.id);
        ...
        if (action === 'list-tasks') {
            appendTerminalLog('[首页动作] 查看任务命令已执行完，上方输出就是当前运行中的任务列表。\n', 'sys', session.id);
            refreshStatusCard();
            return;
        }
        ...
        if (!isLifecycleAction || !window.api?.getDashboardGatewayStatus) {
            ...
            appendTerminalLog('[首页动作] 管理命令已执行完。\n', 'sys', session.id);
            refreshStatusCard();
            return;
        }
        ...
        setTimeout(async () => {
            try {
                ...
                const introMap = {
                    start: '[首页动作] 启动命令已执行完。这表示管理命令本身已经退出，不代表 Gateway 已停止。',
                    stop: '[首页动作] 停止命令已执行完。',
                    restart: '[首页动作] 重启命令已执行完。'
                };
                const suffix = detail ? ` 当前状态：${statusText}；${detail}` : ` 当前状态：${statusText}。`;
                appendTerminalLog(`${introMap[action] || '[首页动作] 管理命令已执行完。'}${suffix}\n`, 'sys', session.id);
            } catch (error) {
                appendTerminalLog(`[首页动作] 管理命令已执行完，但状态刷新失败：${error?.message || error}\n`, 'sys', session.id);
            } finally {
                refreshStatusCard();
            }
        }, 1200);
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        searchTimer = setTimeout(() => {
            ...
            renderPage();
            return;
            // 不再重置整个 DOM，只更新需要更新的内涵！这解决了搜素时的刷新问题！
            renderPage();
        }, 300);
    });
    ...
    function updateSpotlightResults() {
        ...
        if (currentFilteredCommands.length === 0) {
            spotlightResults.innerHTML = '<li class="spotlight-result-item" style="justify-content:center; color:var(--text-muted); cursor:default;">无匹配指令</li>';
            return;
        }
...
function showFormDialog({ title, description = '', fields, confirmText = '保存', cancelText = '取消', onConfirm, onClose }) {
    const overlay = document.createElement('div');
    overlay.className = 'ocp-dialog-overlay';
        ...
        if (field.type === 'select') {
            ...
            return `
                <div class="ocp-dialog-field">
                    ${labelHtml}
                    ${descriptionHtml}
                    <select data-field="${escapeHtml(field.name)}" ${field.readonly ? 'disabled' : ''} class="ocp-dialog-input ocp-dialog-select">
...
// =======================================================
// 模型配置页 (Ported from clawpanel models.js)
// =======================================================
async function renderModelsPage(container) {
    container.innerHTML = `
        <div style="padding:24px 28px">
            ...
            <div style="display:flex;gap:10px;margin-bottom:18px;flex-wrap:wrap;align-items:center">
                <button id="mpBtnAddProvider" style="padding:8px 18px;background:var(--accent,#78dce8);color:#1e1e24;border:none;border-radius:8px;cursor:pointer;font-weight:600">+ 添加服务商</button>
    ...
    function renderDefaultBar() {
        ...
        bar.innerHTML = `
            <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:12px 16px;display:flex;gap:20px;flex-wrap:wrap">
                <span style="color:#8b8b93;font-size:13px">主模型：<span style="color:${primary ? '#a9dc76' : '#ff6188'};font-family:monospace">${primary || '未配置'}</span></span>
                <span style="color:#8b8b93;font-size:13px">备选：<span style="color:#8b8b93">${fallbacks.length ? fallbacks.slice(0,3).join(', ') + (fallbacks.length > 3 ? '...' : '') : '无'}</span></span>
            </div>`;
    }
    ...
    function renderProviders() {
        ...
        const primary = getPrimary();
        if (!keys.length) {
            listEl.innerHTML = '<div style="color:#8b8b93;padding:20px;text-align:center">暂无服务商，点击「+ 添加服务商」开始配置</div>';
            return;
        }
        listEl.innerHTML = keys.map(key => {
            ...
            <div class="mpProvider" data-key="${key}" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:12px;padding:16px 18px;margin-bottom:14px">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;flex-wrap:wrap;gap:8px">
                    <span style="font-weight:600;color:#e0e0e0">${key} <span style="font-size:11px;color:#8b8b93;font-weight:400">${p.api || 'openai'} · ${models.length} 个模型</span></span>
                    <div style="display:flex;gap:7px">
                        <button class="mpEditProv" style="padding:5px 12px;background:rgba(255,255,255,0.07);border:none;border-radius:6px;color:#e0e0e0;cursor:pointer;font-size:12px">编辑</button>
                        <button class="mpAddModel" style="padding:5px 12px;background:rgba(255,255,255,0.07);border:none;border-radius:6px;color:#e0e0e0;cursor:pointer;font-size:12px">+ 模型</button>
                        <button class="mpDelProvider" style="padding:5px 12px;background:#ff618820;border:none;border-radius:6px;color:#ff6188;cursor:pointer;font-size:12px">删除</button>
                    </div>
                </div>
                <div class="mpModels">
                    ${models.map(m => {
                        ...
                        <div class="mpModelRow" data-modelid="${id}" data-full="${full}" style="display:flex;align-items:center;gap:8px;padding:8px 10px;border-radius:7px;margin-bottom:5px;background:${isPrim ? 'rgba(169,220,118,0.08)' : 'rgba(255,255,255,0.02)'};border:1px solid ${isPrim ? '#a9dc7640' : 'rgba(255,255,255,0.05)'}">
                            ...
                            ${isPrim ? '<span style="font-size:11px;background:#a9dc76;color:#1e1e24;padding:1px 8px;border-radius:5px">主模型</span>' : ''}
                            ${!isPrim ? `<button class="mpSetPrimary" style="padding:4px 10px;background:rgba(255,255,255,0.07);border:none;border-radius:5px;color:#8b8b93;cursor:pointer;font-size:11px">设为主模型</button>` : ''}
                            <button class="mpTestModel" style="padding:4px 10px;background:rgba(255,255,255,0.07);border:none;border-radius:5px;color:#78dce8;cursor:pointer;font-size:11px">测试</button>
                            <button class="mpDelModel" style="padding:4px 10px;background:#ff618820;border:none;border-radius:5px;color:#ff6188;cursor:pointer;font-size:11px">删除</button>
                        </div>`;
                    }).join('')}
                </div>
            </div>`;
        ...
        listEl.querySelectorAll('.mpProvider').forEach(section => {
            ...
            section.querySelector('.mpDelProvider').onclick = async () => {
                if (!confirm(`确定删除「${key}」及其所有模型？`)) return;
                    ...
                    testBtn.onclick = async () => {
                        ...
                        try {
                            ...
                            if (res.ok || res.status === 200) {
                                testBtn.textContent = `✅ ${(ms/1000).toFixed(1)}s`;
                                testBtn.style.color = '#a9dc76';
                            } else {
                                testBtn.textContent = `❌ ${res.status}`;
                                testBtn.style.color = '#ff6188';
                            }
                        } catch(e) {
                            testBtn.textContent = '❌ 超时';
                            testBtn.style.color = '#ff6188';
                        }
        ...
        overlay.innerHTML = `
            <div style="background:#2c2c3a;border-radius:14px;padding:24px;min-width:380px;max-width:540px;width:90%">
                ...
                <div style="display:flex;flex-direction:column;gap:12px">
                    <div><label style="color:#8b8b93;font-size:13px;display:block;margin-bottom:4px">服务商名称（唯一标识）</label>
                    ...
                    <div><label style="color:#8b8b93;font-size:13px;display:block;margin-bottom:4px">API 类型</label>
                        <select id="mpProvApi" style="width:100%;padding:8px 10px;background:#1e1e24;border:1px solid rgba(255,255,255,0.1);border-radius:7px;color:#e0e0e0;font-size:13px">
                            <option value="openai-completions" ${existingData?.api === 'openai-completions' ? 'selected' : ''}>OpenAI-兼容 (ChatCompletions)</option>
    ...
    function showAddModelModal(providerKey) {
        ...
        overlay.innerHTML = `
            <div style="background:#2c2c3a;border-radius:14px;padding:24px;min-width:360px">
                <h3 style="color:#e0e0e0;margin:0 0 14px">添加模型到 ${providerKey}</h3>
                <input id="mpNewModelId" placeholder="模型 ID（如 gpt-4o, claude-3-5-sonnet-20241022）" style="width:100%;padding:8px 10px;background:#1e1e24;border:1px solid rgba(255,255,255,0.1);border-radius:7px;color:#e0e0e0;font-size:13px;box-sizing:border-box">
                <div style="margin-top:14px;display:flex;justify-content:flex-end;gap:10px">
                    ...
                    <button id="mpAddConfirm" style="padding:7px 16px;background:#78dce8;color:#1e1e24;border:none;border-radius:7px;cursor:pointer;font-weight:600">添加</button>
                </div>
            </div>`;
        document.body.appendChild(overlay);
...
// =======================================================
// 日志查看页 (Ported from clawpanel logs.js)
// =======================================================
async function renderLogsPage(container) {
    ...
    async function loadFile(filePath) {
        ...
        try {
            ...
            if (!raw.trim()) { content.textContent = '（日志文件为空）'; return; }
            // Parse JSONL
            ...
            for (const line of lines) {
                try {
                    ...
                    formatted += `\n[${roleColor === '#78dce8' ? '👤' : roleColor === '#a9dc76' ? '🤖' : '⚙️'} ${role.toUpperCase()}]\n${typeof text === 'string' ? text : JSON.stringify(text, null, 2)}\n${'─'.repeat(60)}\n`;
                } catch {
                    formatted += line + '\n';
                }
            }
    ...
    async function loadGatewayLog() {
        currentMode = 'gateway';
        content.textContent = '加载 Gateway 日志...';
        try {
            const raw = await window.api.readGatewayLog(300);
            if (!raw.trim()) {
                content.textContent = '（Gateway 日志为空或未找到日志文件）\n\n可能的位置：\n~/.openclaw/gateway.log\n~/.openclaw/logs/gateway.log';
                return;
            }
            // Colorize log lines
            const lines = raw.split('\n').map(line => {
                if (line.includes('[ERROR]') || line.includes('ERROR')) return '[❌] ' + line;
                if (line.includes('[WARN]') || line.includes('WARN')) return '[⚠️] ' + line;
                if (line.includes('[INFO]') || line.includes('INFO')) return '[ℹ️] ' + line;
                return line;
            });
            content.textContent = lines.join('\n');
            content.scrollTop = content.scrollHeight;
        } catch(e) {
            content.textContent = 'Gateway 日志加载失败: ' + e.message;
        }
    }
...
// =======================================================
// Gateway 配置页 (Ported from clawpanel gateway.js)
// =======================================================
        ...
        <div style="display:flex;flex-direction:column;gap:16px;max-width:560px">
            ...
            <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.07);border-radius:12px;padding:18px">
                ...
                <div>
                    ...
                    <p style="font-size:11px;color:#8b8b93;margin:6px 0 0">这里会同步写回本地配置文件中的 \`agents.defaults.tools_mode\`。</p>
                </div>
            </div>
...
async function renderAgentsPage(container) {
    container.innerHTML = `
        <div style="padding:24px 28px;height:100%;display:flex;flex-direction:column;box-sizing:border-box">
            ...
            <p class="page-desc">查看和编辑 Agent 配置文件（IDENTITY、SOUL、USER）。</p>
            <div style="display:flex;gap:16px;flex:1;min-height:0">
                ...
                <div style="flex:1;min-width:0">
                    <div id="agFilePanel" style="height:100%;display:flex;flex-direction:column">
                        <div style="color:#8b8b93;padding:20px;text-align:center">请从左侧选择 Agent</div>
                    </div>
                </div>
            </div>
        </div>`;
    ...
    async function loadFileContent() {
        ...
        try {
            ...
            if (content === null) {
                editor.innerHTML = `
                    <div style="color:#8b8b93;font-size:13px;padding:10px;margin-bottom:10px">文件不存在，可以在此创建：</div>
                    <textarea id="agTextarea" style="flex:1;width:100%;background:#141418;border:1px solid rgba(255,255,255,0.08);border-radius:8px;color:#c0c0c0;font-family:monospace;font-size:12px;padding:12px;resize:none;box-sizing:border-box;min-height:240px" placeholder="在此输入内容..."></textarea>
        ...
        try {
            ...
            editor.innerHTML = `
                <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:10px">
                    ...
                    <div style="font-size:12px;color:#8f98ab">${content === null ? '文件不存在，保存后会自动创建' : '保存后直接写回本地文件'}</div>
                </div>
...
async function renderModelsPage(container) {
        ...
        bar.innerHTML = `
            <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:14px 16px;display:flex;gap:20px;flex-wrap:wrap;align-items:center">
                ...
                <div style="display:flex;flex-direction:column;gap:6px">
                    ...
                    <span style="color:#d7dce5;font-size:13px">勾选模型后点“批量测试”；如果没有勾选，则默认测试当前服务商全部模型。</span>
                </div>
            </div>
        `;
    }
    ...
    function getProviderSelectionText(providerKey) {
        ...
        return selectedCount ? `已勾选 ${selectedCount} 个模型` : '不勾选时默认测试这个 provider 的全部模型。';
    }
...
async function renderModelsPageV2(container) {
    ...
    if (pageDescEl) {
        pageDescEl.textContent = '把服务商、模型和默认模型拆成清晰的列表与明细结构，先看清状态，再决定新增、编辑、导入或测试。获取模型列表会优先走标准接口拉取，没有标准接口时再适配官方专有列表接口，再不行就使用内置静态模型目录作为回退。';
    }

    container.querySelector('.page-title')?.replaceChildren(document.createTextNode('模型配置'));
    pageDescEl?.replaceChildren(document.createTextNode('把服务商、模型和默认模型拆成清晰的列表与明细结构，先看清状态，再决定新增、编辑、导入或测试。获取模型列表会优先走标准接口拉取，没有标准接口时再适配官方专有列表接口，仍不可用时回退到内置静态目录。'));
    ...
    function getMergedProviderEntries() {
        ...
        return Array.from(allKeys).map((providerKey) => {
            ...
            return {
                ...
                models: mergeModelRecords(localProvider?.models || [], runtimeProvider.models || [], api),
    ...
    function getGroupMeta(sourceKey) {
        const groups = {
            ...
            reference: {
                title: '引用中的模型',
                desc: '当前配置已经引用这些模型，即使 provider 还未完整写入。'
            }
        };
        if (sourceKey === 'auth-oauth') {
            return {
                title: '登录态来源',
                desc: '来自本地凭据目录，主要用于补充当前可用的 provider 与模型上下文。'
            };
        }
    ...
    function getProviderSelectionLabel(providerKey) {
        const models = collectProviderModels(providerKey);
        const selectedCount = models.filter((item) => state.selectedModels.has(item.full)).length;
        return selectedCount ? `已勾选 ${selectedCount} 个模型` : '不勾选时默认测试当前列表里的全部模型。';
    }
    ...
    async function persistModelConfig(options = {}) {
        ...
        try {
            ...
            setPageStatus(`${failurePrefix}: ${error?.message || error}`, '#ff6188');
            return false;
        } finally {
            ...
            <div style="width:min(860px,96vw);max-height:82vh;background:#1c212d;border:1px solid rgba(255,255,255,0.08);border-radius:18px;padding:20px;box-shadow:0 30px 60px rgba(0,0,0,0.38);display:flex;flex-direction:column;gap:14px">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap">
                    <div>
                        ...
                        <div style="color:#8f98ab;font-size:13px">共获取 ${remoteModels.length} 个模型，勾选后会写入本地 openclaw.json。</div>
                    </div>
    ...
    function renderProviders() {
            ...
            <div class="models-provider-detail" data-provider-card="${escapeHtml(provider.key)}">
                <div class="models-detail-header">
                    <div class="models-detail-heading">
                        ...
                        <p class="models-detail-desc">${escapeHtml(provider.inConfig ? '先确认服务商配置，再在下方维护模型和默认项。' : '当前服务商还未写入本地配置，可以先写入配置，再继续编辑或导入模型。')}</p>
                    </div>
                ...
                <section class="models-model-section">
                    <div class="models-model-section-head">
                        <div>
                            ...
                            <div class="models-model-section-desc">行内只保留一个高频动作，测试和删除降级到弱操作区。</div>
                        </div>
                        ${models.length ? `
                            ...
                            <div class="mpProviderCardV2 mpProvider models-provider-card" data-provider="${escapeHtml(provider.key)}" data-provider-card="${escapeHtml(provider.key)}" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:16px;padding:18px 18px 14px;margin-bottom:16px">
                                <div class="models-provider-header" style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;flex-wrap:wrap;gap:12px">
                                    <div class="models-provider-meta" style="display:flex;flex-direction:column;gap:6px">
                                        <div class="models-provider-meta-row" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
                                            ...
                                            <span style="font-size:11px;color:#8f98ab">${escapeHtml(apiLabelMap[provider.api] || provider.api || 'OpenAI 兼容')}</span>
                                            <span style="font-size:11px;color:#8f98ab">${models.length} 个模型</span>
                                            <span style="font-size:11px;color:${provider.inConfig ? '#a9dc76' : '#78dce8'}">${provider.inConfig ? '已写入配置' : '仅读目录'}</span>
                                ...
                                ${models.length ? `
                                    <div class="models-provider-selection" style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:10px;padding:10px 12px;border-radius:12px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.05)">
                                        ...
                                        <span style="font-size:12px;color:#8f98ab">${selectedCount ? `已勾选 ${selectedCount} 个模型` : '不勾选时默认测试这个 provider 的全部模型。'}</span>
                                    </div>
                                ` : ''}
                                <div class="models-provider-list" style="display:flex;flex-direction:column;gap:8px">
                                    ${models.length ? models.map((model) => {
                                        ...
                                        `;
                                    }).join('') : '<div style="padding:16px;border:1px dashed rgba(255,255,255,0.08);border-radius:12px;color:#8f98ab;text-align:center">当前 provider 还没有已收录模型，可以点“获取列表”从供应商 API 拉取。</div>'}
                                </div>
                            </div>
                        `;
                    }).join('')}
        ...
        listEl.querySelectorAll('[data-provider-card]').forEach((card) => {
            ...
            if (!intro) {
                ...
                intro.innerHTML = `
                    <span style="font-size:12px;color:#8f98ab">${escapeHtml(inConfig ? '这里展示服务商说明与凭据状态，操作按钮已放到下方工具区。' : '当前服务商尚未写入配置，可以先看说明，再在下方执行写入或拉取模型。')}</span>
                    <span style="font-size:11px;color:#8f98ab">${escapeHtml(modelCount ? `当前共 ${modelCount} 个模型${selectedCount ? `，已勾选 ${selectedCount} 个` : ''}。` : '当前还没有模型，可先获取列表或手动添加模型。')}</span>
                `;
            ...
            if (!selection) {
                ...
                selection.innerHTML = `
                    <div class="models-provider-selection-copy" style="display:flex;flex-direction:column;gap:6px;min-width:220px">
                        ...
                        <span style="font-size:12px;color:#8f98ab">可以先获取官方模型列表或手动添加模型。</span>
                    </div>
                `;
...
async function renderLogsPage(container) {
    ...
    container.innerHTML = `
        <div style="padding:24px 28px;height:100%;display:flex;flex-direction:column;box-sizing:border-box">
            <h2 class="page-title">日志查看</h2>
            <p class="page-desc">查看 OpenClaw 各服务日志、配置审计日志和 Agent 会话记录。</p>
            ...
            <div style="font-size:12px;color:#8f98ab;margin-bottom:10px" id="lgStatus">正在加载日志...</div>
    ...
    container.querySelector('.page-title')?.replaceChildren(document.createTextNode('日志查看'));
    container.querySelector('.page-desc')?.replaceChildren(document.createTextNode('查看 OpenClaw 服务日志、配置审计日志和 Agent 会话记录。'));
    if (searchInput) searchInput.placeholder = '搜索日志...';
    if (refreshBtn) refreshBtn.textContent = '刷新';
    if (autoScroll?.parentElement) autoScroll.parentElement.lastChild.textContent = ' 自动滚动';
    ...
    tabBar.innerHTML = serviceTabs.map((tab) => `
        <button
            ...
            id="${tab.key === 'gateway' ? 'lgGatewayBtn' : ''}"
    ...
    const logTabLabels = {
        gateway: 'Gateway 日志',
        'gateway-err': 'Gateway 错误',
        guardian: '守护进程',
        'guardian-backup': '备份日志',
        'config-audit': '审计日志',
        session: 'Agent 会话'
    };
    tabBar.querySelectorAll('.lgTabBtn').forEach((button) => {
        const label = logTabLabels[button.dataset.tab];
    ...
    function applySearchAndRender() {
        ...
        if (autoScroll.checked) content.scrollTop = content.scrollHeight;
    }
    ...
    async function loadServiceLog() {
        ...
        const raw = await window.api.readServiceLog(state.activeTab, 400);
        state.renderedText = raw && raw.trim()
            ? raw
            : `${active?.label || '当前日志'} 暂无内容。\n\n已检查 ~/.openclaw/logs 和 ~/.openclaw 根目录下的对应文件。`;
...
async function renderGatewayPage(container) {
    ...
    const toolModeCards = {
        full: {
            title: '完整权限',
            desc: 'Agent 可使用所有工具，适合本机受控环境，推荐作为默认模式。',
                ...
                <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.07);border-radius:16px;padding:18px">
                    ...
                    <div style="display:flex;flex-direction:column;gap:12px">
                        ...
                        <div>
                            ...
                            <div style="margin-top:6px;color:#8f98ab;font-size:12px;line-height:1.6">仅本机使用最安全；如果手机、平板也要访问，再切到局域网共享。</div>
                        </div>
                    </div>
                </div>
                <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.07);border-radius:16px;padding:18px">
