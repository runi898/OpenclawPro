...
const WEIXIN_CHANNEL_STATUS_COMMAND = 'openclaw channels status --probe';
    ...
    function isRecognizedWeixinLoginLink(value) {
        const link = String(value || '').trim();
        if (!link) return false;
        if (/^(?:weixin:\/\/|wx:\/\/)/i.test(link)) return true;
        try {
            const parsed = new URL(link);
            const host = String(parsed.hostname || '').toLowerCase();
            const pathname = String(parsed.pathname || '');
            if (host === 'liteapp.weixin.qq.com' && /^\/q\//.test(pathname)) {
                return true;
            }
        } catch (_) {
            return false;
    ...
    function extractLatestLoginLink(text) {
        ...
        const filtered = matches.filter((item) => isRecognizedWeixinLoginLink(item));
    ...
    function hasWeixinFatalCrash(text) {
        const source = String(text || '');
        const lower = source.toLowerCase();
        if (!source.trim()) return false;
        if (/warning|deprecationwarning|config warnings/i.test(lower)) return false;
        return /fatal error|javascript stack trace|native stack trace|node::abort|v8::|exception code|crash/i.test(source);
    }
    ...
    function parseWeixinLoginSignal(text) {
        ...
        const loginSuccess = /登录成功|login success|认证成功|authorized successfully|已完成扫码|登录完成|连接成功|与微信连接成功|wechat connected|connected to wechat|connected successfully/i.test(source);
        const configOverwritten = /config overwrite:/i.test(source);
        const restartSeen = /gateway.*restart|自动重启.*gateway|restarting gateway|gateway restart/i.test(source);
        const hardFailure = hasWeixinFatalCrash(source)
            || (
                !/warning|deprecationwarning|duplicate plugin|config warnings/i.test(lower)
    ...
    function hasWeixinAcceptedLoginSignal(parsed = {}, fallbackState = state) {
        return Boolean(
            parsed.loginSuccess
            || parsed.restartSeen
            || (
                parsed.configOverwritten
                && (
                    parsed.link
                    || parsed.asciiQr
                    || parsed.scanned
                    || fallbackState?.weixinLoginLink
                    || fallbackState?.weixinLoginAsciiQr
                )
            )
        );
    }
    ...
    function parseWeixinRuntimeStatus(text) {
        ...
        lines.forEach((line, index) => {
            if (/(openclaw-weixin|个人微信(?:\s*v?1)?|微信|wechat)/i.test(line)) {
        ...
        const connected = mentionsWeixin
            && /(connected|online|ready|logged in|authenticated|已连接|已登录|登录成功|连接成功|与微信连接成功)/i.test(relevantBlock)
    ...
    function reconcileWeixinLoginStateFromRuntimeLog(nextState = state) {
        const runtimeLog = [
            String(nextState.weixinStatusCheckRawLog || ''),
            String(nextState.weixinLoginRawLog || '')
        ]
        ...
        const runtimeParsed = parseWeixinRuntimeStatus(runtimeLog);
        const loginParsed = parseWeixinLoginSignal(runtimeLog);
        const acceptedLogin = hasWeixinAcceptedLoginSignal(loginParsed, nextState);

        if (runtimeParsed.connected) {
            nextState.weixinLoginPhase = 'success';
            nextState.weixinLoginSummary = '个人微信已连接成功';
            nextState.weixinLoginDetail = '已从运行状态日志中识别到连接成功信号，当前接入已经完成。';
            nextState.status = '个人微信已确认接入成功。';
            nextState.weixinLoginFinished = true;
            nextState.weixinLoginExpired = false;
            return;
        }

        if (
            acceptedLogin
            && !['success', 'failure', 'expired'].includes(String(nextState.weixinLoginPhase || ''))
        ) {
            nextState.weixinLoginPhase = 'checking';
            nextState.weixinLoginSummary = '已识别到扫码成功，正在确认连接';
            nextState.weixinLoginDetail = loginParsed.loginSuccess
                ? '运行状态日志已经出现连接成功信号，界面正在等待最终确认。'
                : '运行状态日志已出现配置写回或重启信号，通常说明扫码已经被接受。';
            nextState.status = '已识别到个人微信扫码成功，正在自动确认连接状态。';
        }
    }
    ...
    function getWeixinStepTimeline(weixinState = {}) {
        const phase = String(weixinState.weixinLoginPhase || 'idle').trim();
        const flow = String(weixinState.weixinLoginFlow || '').trim();
        const timeline = [
            { key: 'prepare', title: '准备环境', detail: flow === 'install' ? '安装或升级插件' : '检查插件与接入条件', state: 'pending' },
            { key: 'qr', title: '获取二维码', detail: '生成二维码或扫码链接', state: 'pending' },
            { key: 'confirm', title: '手机确认', detail: '在手机侧完成授权', state: 'pending' },
            { key: 'done', title: '完成接入', detail: '检查状态并确认可用', state: 'pending' }
        ];

        if (phase === 'idle') {
            timeline[0].state = 'active';
            return timeline;
        }
        ...
        if (phase === 'expired') {
            timeline[1].state = 'danger';
            return timeline;
        }

        timeline[1].state = 'done';
        ...
        if (phase === 'failure') {
            timeline[3].state = 'danger';
            return timeline;
        }

        if (phase === 'success') {
            timeline[3].state = 'done';
            return timeline;
        }
    ...
    function buildWeixinUserFacingRuntimeLog(logText = '', options = {}) {
        ...
        if (hasWeixinFatalCrash(source)) {
            return '扫码 CLI 已出现崩溃堆栈，二维码未能生成；请展开原始日志检查后重试。';
        }
    ...
    const CHANNEL_REGISTRY = {
        ...
        feishu: {
            ...
            builtIn: true,
            installCommand: 'openclaw plugins install @larksuite/openclaw-lark@latest',
        ...
        'openclaw-weixin': {
            ...
            installCommand: WEIXIN_AUTO_INSTALL_COMMAND,
            guide: [
        ...
        dingtalk: {
            ...
            installCommand: 'openclaw plugins install @dingtalk-real-ai/dingtalk-connector@latest',
            pluginRequired: '@dingtalk-real-ai/dingtalk-connector@latest',
            guide: [
                '前往钉钉开放平台创建企业内部应用，并添加机器人能力。',
                '安装命令为 openclaw plugins install @dingtalk-real-ai/dingtalk-connector@latest。',
        ...
        discord: {
            ...
            installCommand: 'Bot Token 接入，无需额外插件安装。',
            guide: [
        ...
        slack: {
            label: 'Slack',
            desc: '支持 Socket Mode 与 HTTP 模式的 Slack Bot 接入。',
            installCommand: 'Socket / HTTP 直连接入，无需单独安装插件。',
            guide: [
                '在 Slack API 控制台创建应用，并为 Bot 添加所需权限。',
                'Socket Mode 需要 Bot Token + App Token；HTTP 模式需要 Bot Token + Signing Secret。',
                'Team ID、Webhook Path、Allow From 可按实际环境补充。',
                '保存前建议先按实际模式补齐必填项，再做凭证校验。'
            ],
            defaults: {
                enabled: true,
                mode: 'socket',
            ...
            fields: [
                { key: 'mode', label: '接入模式', type: 'select', options: [['socket', 'Socket Mode'], ['http', 'HTTP 模式']], required: true },
                { key: 'botToken', label: 'Bot Token', placeholder: 'xoxb-xxxxxxxxxxxx', secret: true, required: true },
                { key: 'appToken', label: 'App Token', placeholder: 'xapp-xxxxxxxxxxxx', secret: true },
                { key: 'signingSecret', label: 'Signing Secret', placeholder: 'HTTP 模式使用', secret: true },
                { key: 'teamId', label: 'Team ID', placeholder: '可选' },
                { key: 'webhookPath', label: 'Webhook Path', placeholder: '/slack/events' },
                { key: 'dmPolicy', label: '私聊策略', type: 'select', options: [['', '默认'], ['allow', 'allow'], ['deny', 'deny']] },
                { key: 'groupPolicy', label: '群聊策略', type: 'select', options: [['', '默认'], ['all', 'all'], ['mentioned', 'mentioned'], ['allowlist', 'allowlist']] },
                { key: 'allowFrom', label: '允许来源', kind: 'list', placeholder: '多个值用逗号分隔' }
            ],
            pairing: false
        },
        msteams: {
            label: 'Microsoft Teams',
            desc: 'Azure AD / Bot Framework 渠道接入。',
            installCommand: 'openclaw plugins install @openclaw/msteams@latest',
            pluginRequired: '@openclaw/msteams@latest',
            guide: [
                '在 Azure AD / Microsoft Teams 开发后台创建 Bot 应用。',
                '准备 App ID、App Password，必要时补充 Tenant ID 与 Bot Endpoint。',
                '如需通过 Webhook 接入，可填写 Webhook Path。',
                '保存后再进行实际路由测试。'
            ],
            defaults: {
                enabled: true,
            ...
            fields: [
                ...
                { key: 'tenantId', label: 'Tenant ID', placeholder: '可选' },
                { key: 'botEndpoint', label: 'Bot Endpoint', placeholder: 'https://example.com/api/teams/messages' },
                { key: 'webhookPath', label: 'Webhook Path', placeholder: '/msteams/messages' },
                { key: 'dmPolicy', label: '私聊策略', type: 'select', options: [['', '默认'], ['allow', 'allow'], ['deny', 'deny']] },
                { key: 'groupPolicy', label: '群聊策略', type: 'select', options: [['', '默认'], ['all', 'all'], ['mentioned', 'mentioned'], ['allowlist', 'allowlist']] },
                { key: 'allowFrom', label: '允许来源', kind: 'list', placeholder: '多个值用逗号分隔' }
            ],
        ...
        signal: {
            label: 'Signal',
            desc: '通过 Signal CLI / HTTP 网关接入消息通道。',
            installCommand: '依赖 Signal CLI 或 HTTP 网关，无单独插件包。',
            guide: [
                '准备可用的 Signal CLI 账号或 HTTP 网关服务。',
                '至少填写账号标识；如走本地 CLI，可额外提供 CLI 路径。',
                '如走 HTTP 模式，可补充 URL / Host / Port。',
                '保存后建议先做消息探测，再投入生产。'
            ],
            defaults: {
                enabled: true,
            ...
            fields: [
                { key: 'account', label: '账号标识', placeholder: '+8613812345678', required: true },
                { key: 'cliPath', label: 'CLI Path', placeholder: 'signal-cli 路径（可选）' },
                { key: 'httpUrl', label: 'HTTP URL', placeholder: 'http://127.0.0.1:8080' },
                { key: 'httpHost', label: 'HTTP Host', placeholder: '127.0.0.1' },
                { key: 'httpPort', label: 'HTTP Port', placeholder: '8080' },
                { key: 'dmPolicy', label: '私聊策略', type: 'select', options: [['', '默认'], ['allow', 'allow'], ['deny', 'deny']] },
                { key: 'groupPolicy', label: '群聊策略', type: 'select', options: [['', '默认'], ['all', 'all'], ['mentioned', 'mentioned'], ['allowlist', 'allowlist']] },
                { key: 'allowFrom', label: '允许来源', kind: 'list', placeholder: '多个值用逗号分隔' }
            ],
        ...
        matrix: {
            label: 'Matrix',
            desc: 'Matrix Homeserver + Bot 令牌方式接入。',
            installCommand: 'openclaw plugins install @openclaw/matrix@latest',
            pluginRequired: '@openclaw/matrix@latest',
            guide: [
                '准备 Homeserver 地址与 Bot 账号。',
                'Access Token、User ID、Password 可按环境任选其一或组合使用。',
                '如需端到端加密，可补充 Device ID 与 E2EE 配置。',
                '保存后建议先在测试房间验证收发。'
            ],
            defaults: {
                enabled: true,
            ...
            fields: [
                ...
                { key: 'accessToken', label: 'Access Token', placeholder: 'syt_xxxxx', secret: true },
                { key: 'userId', label: 'User ID', placeholder: '@bot:matrix.org' },
                { key: 'password', label: 'Password', placeholder: '可选', secret: true },
                { key: 'deviceId', label: 'Device ID', placeholder: 'CLAWPANEL' },
                { key: 'e2ee', label: 'E2EE', type: 'select', options: [['', '默认'], ['true', '启用'], ['false', '关闭']] },
                { key: 'dmPolicy', label: '私聊策略', type: 'select', options: [['', '默认'], ['allow', 'allow'], ['deny', 'deny']] },
                { key: 'groupPolicy', label: '群聊策略', type: 'select', options: [['', '默认'], ['all', 'all'], ['mentioned', 'mentioned'], ['allowlist', 'allowlist']] },
                { key: 'allowFrom', label: '允许来源', kind: 'list', placeholder: '多个值用逗号分隔' }
            ],
    ...
    const PRIMARY_CHANNEL_ORDER = ['qqbot', 'dingtalk', 'feishu', 'telegram', 'discord', 'slack', 'msteams', 'signal', 'matrix', 'wecom', 'openclaw-weixin'];
    ...
    function inferChannelProfileKey(channelName, channelConfig = {}) {
        ...
        if (source.botToken) {
            if (source.mode || source.appToken || source.signingSecret || source.teamId || source.webhookPath) {
                return 'slack';
            }
            return 'telegram';
        }
        ...
        if (
            ...
            || source.userId
            || source.deviceId
            || Object.prototype.hasOwnProperty.call(source, 'e2ee')
        ) {
            return 'matrix';
        }
    ...
    function normalizeChannelAccessProbeResult(result, channelName) {
        ...
        const shouldInstallFirst = !configured && !isBuiltIn && Boolean(profile?.installCommand || profile?.pluginRequired);
        return {
            state: configured ? 'configured' : 'unknown',
            badge: configured ? '已接入' : (shouldInstallFirst ? '待安装' : '待接入'),
            button: configured ? '编辑配置' : (shouldInstallFirst ? '安装并接入' : '立即接入'),
            tone: configured ? 'success' : (shouldInstallFirst ? 'warning' : 'info'),
            detail: configured
                ? `已写入配置，点击可继续编辑 ${profile.label || channelName}。`
                : (shouldInstallFirst
                    ? '当前还没有拿到明确的环境探测结果，先执行安装流程更稳妥。'
                    : '点击主按钮打开接入弹窗，后续可接后端环境检测接口。'),
            hint: configured
                ? '参数编辑与凭证校验会在弹窗里完成。'
                : (shouldInstallFirst
                    ? '确认后会先弹出安装源选择，再继续本机安装 / 升级流程。'
                    : '当前状态未确认环境安装情况，先按接入流程完成配置。'),
            action: configured ? 'edit' : (shouldInstallFirst ? 'install' : 'create'),
            followupAction: configured ? 'edit' : 'create',
            target: channelName
        };
    }
    ...
    function getChannelInstallCommandText(profile = {}) {
        const explicit = String(profile?.installCommand || '').trim();
        if (explicit) return explicit;
        const pluginRequired = String(profile?.pluginRequired || '').trim();
        if (pluginRequired) return `openclaw plugins install ${pluginRequired}`;
        if (profile?.builtIn) return '当前版本通常内置，无需额外安装';
        return '';
    }

    function channelSupportsMultipleInstances(channelName = '') {
        const normalized = normalizeChannelKey(channelName) === 'dingtalk-connector'
            ? 'dingtalk'
            : normalizeChannelKey(channelName);
        return ['qqbot', 'feishu', 'dingtalk'].includes(normalized);
    }

    function buildConfiguredChannelGroups(state, channelKeys = []) {
        const grouped = new Map();
        channelKeys.forEach((channelName) => {
            const channelConfig = ensureObject(state.config?.channels?.[channelName]);
            const profileKey = inferChannelProfileKey(channelName, channelConfig);
            const profile = getChannelProfile(channelName, channelConfig);
            const probe = getChannelAccessProbe(state, channelName);
            ...
            entry.items.push({
                ...
                probe,
                actionState,
                boundAgent: getBoundAgent(state.config, channelName)
            });
            grouped.set(profileKey, entry);
        });

        return sortChannelKeys(Array.from(grouped.keys())).map((profileKey) => {
            const group = grouped.get(profileKey);
            const selectedItem = group.items.find((item) => normalizeChannelKey(item.channelName) === normalizeChannelKey(state.selected));
            group.primaryItem = selectedItem || group.items[0];
            group.anyEnabled = group.items.some((item) => item.channelConfig.enabled !== false);
            group.selected = group.items.some((item) => normalizeChannelKey(item.channelName) === normalizeChannelKey(state.selected));
            return group;
        });
    }

    function renderConfiguredChannelGroupCard(group, state) {
        const profile = group.profile || {};
        const primaryItem = group.primaryItem || group.items[0];
        const monogram = getChannelMonogram(profile, group.profileKey);
        const installCommand = getChannelInstallCommandText(profile);
        const badgeTone = primaryItem?.actionState?.tone || (group.anyEnabled ? 'success' : 'muted');
        const badgeText = group.items.length > 1
            ? `${group.items.length} 个实例`
            : (primaryItem?.actionState?.badge || (group.anyEnabled ? '已接入' : '已停用'));
        const rowsHtml = group.items.map((item) => `
            <div class="channel-instance-row ${normalizeChannelKey(item.channelName) === normalizeChannelKey(state.selected) ? 'is-selected' : ''}" data-channel-select="${esc(item.channelName)}">
                <div class="channel-instance-main">
                    <div class="channel-instance-title">${esc(item.channelName)}</div>
                    <div class="channel-instance-meta">Agent：${esc(item.boundAgent || 'main')} · ${esc(item.actionState?.detail || profile.desc || '已接入')}</div>
                </div>
                <div class="channel-instance-actions">
                    <span class="ocp-pill ${esc(item.actionState?.tone || 'muted')}">${esc(item.actionState?.badge || (item.channelConfig.enabled === false ? '已停用' : '已接入'))}</span>
                    ${channelSupportsMultipleInstances(group.profileKey) ? `<button class="ocp-btn sm" data-channel-create-preset="${esc(group.profileKey)}">新增账号</button>` : ''}
                    <button class="ocp-btn sm" data-channel-edit="${esc(item.channelName)}">编辑</button>
                    <button class="ocp-btn sm" data-channel-toggle="${esc(item.channelName)}">${item.channelConfig.enabled === false ? '启用' : '停用'}</button>
                    <button class="ocp-btn sm danger" data-channel-remove="${esc(item.channelName)}">移除</button>
                </div>
            </div>
        `).join('');

        return `
            <article class="platform-card ${group.anyEnabled ? 'active' : 'inactive'} ${group.selected ? 'is-selected' : ''}">
                <div class="platform-card-header">
                    <div class="platform-emoji" aria-hidden="true">${esc(monogram)}</div>
                    <div class="platform-name-block">
                        <div class="platform-name">${esc(profile.label || group.profileKey)}</div>
                        <div class="platform-meta">${esc(profile.desc || '消息渠道接入')}</div>
                    </div>
                    <span class="ocp-pill ${esc(badgeTone)}">${esc(badgeText)}</span>
                    <span class="platform-status-dot ${group.anyEnabled ? 'on' : 'off'}"></span>
                </div>
                <div class="platform-card-body">
                    ${installCommand ? `<div class="platform-install-code"><span>安装代码</span><code>${esc(installCommand)}</code></div>` : ''}
                    <div class="channel-instance-list">
                        ${rowsHtml}
                    </div>
                </div>
            </article>
    ...
    function renderAvailableChannelPick(channelName, state, groupsByProfile = {}) {
        const profile = getChannelProfile(channelName);
        const group = groupsByProfile[normalizeChannelKey(channelName)] || null;
        const targetChannel = group?.primaryItem?.channelName || channelName;
        const targetConfig = ensureObject(group?.primaryItem?.channelConfig);
        const targetProfile = group?.primaryItem?.profile || profile;
        const targetProbe = group?.primaryItem?.probe || getChannelAccessProbe(state, targetChannel);
        const actionState = group
            ? (group.primaryItem?.actionState || getChannelAccessActionState(state, targetChannel, {
                profile: targetProfile,
                channelConfig: targetConfig,
                probe: targetProbe
            }))
            : getChannelAccessActionState(state, channelName, {
                profile,
                channelConfig: {},
                probe: targetProbe
            });
        const installCommand = getChannelInstallCommandText(profile);
        const configured = Boolean(group?.items?.length);
        const badgeHtml = [
            configured ? '<span class="platform-pick-badge">已接入</span>' : '',
            installCommand ? '<span class="platform-pick-badge is-code">含安装代码</span>' : ''
        ].filter(Boolean).join('');

        return `
            <div class="platform-pick-shell" data-channel-select="${esc(targetChannel)}">
                <button
                    type="button"
                    class="platform-pick ${configured ? 'configured' : ''}"
                    data-channel-main-action="${esc(targetChannel)}"
                    data-channel-main-kind="${esc(configured ? 'edit' : (actionState.action || 'create'))}">
                    <span class="platform-emoji" aria-hidden="true">${esc(getChannelMonogram(profile, channelName))}</span>
                    <span class="platform-pick-name">${esc(profile.label || channelName)}</span>
                    <span class="platform-pick-desc">${esc(profile.desc || '选择后开始接入')}</span>
                    ${badgeHtml ? `<span class="platform-pick-badges">${badgeHtml}</span>` : ''}
                    ${installCommand ? `<span class="platform-install-code compact"><span>安装代码</span><code>${esc(installCommand)}</code></span>` : ''}
                </button>
            </div>
        `;
    }

    function renderChannelAgentBindingsPanel(state, groups) {
        const configuredItems = groups.flatMap((group) => group.items || []);
        const agentIds = Array.from(new Set(['main', ...ensureArray(state.agents), ...configuredItems.map((item) => item.boundAgent || 'main')]));
        return agentIds.map((agentId) => {
            const rows = configuredItems.filter((item) => String(item.boundAgent || 'main') === String(agentId));
            return `
                <section class="agent-binding-card">
                    <div class="agent-binding-card-head">
                        <div>
                            <div class="agent-binding-title">${esc(agentId)}</div>
                            <div class="ocp-row-meta">${rows.length ? `当前绑定 ${rows.length} 个通信渠道` : '当前还没有渠道绑定到这个 Agent。'}</div>
                        </div>
                    </div>
                        ...
                        ${rows.length ? rows.map((item) => `
                            <div class="agent-binding-row">
                                <div class="agent-binding-row-main">
                                    <div class="agent-binding-channel">${esc(item.profile?.label || item.channelName)}</div>
                                    <div class="ocp-row-meta">${esc(item.channelName)} · ${esc(item.actionState?.badge || '已接入')}</div>
                                </div>
                                <div class="agent-binding-row-actions">
                                    <button class="ocp-btn sm" data-channel-edit="${esc(item.channelName)}">编辑</button>
                                </div>
                            </div>
                        `).join('') : '<div class="ocp-empty">暂无绑定。</div>'}
                    </div>
                </section>
            `;
        }).join('');
    ...
    async function renderChannelsPage(container) {
        container.innerHTML = `<div class="ocp-shell channels-compact-shell"><div class="ocp-card channels-page-shell"><div class="ocp-card">加载中...</div></div></div>`;
        const state = {
            config: {},
            agents: ['main'],
            selected: '',
            ...
            weixinStatusCheckBackground: false,
        ...
        function resetWeixinLoginState(options = {}) {
            ...
            state.weixinStatusCheckBackground = false;
        ...
        function updateWeixinLoginView(patch = {}) {
            const previousPhase = String(state.weixinLoginPhase || '');
            Object.assign(state, patch);
            reconcileWeixinLoginStateFromRuntimeLog(state);
            ...
            refreshWeixinDialogRuntimeUI();
            if (previousPhase !== 'success' && state.weixinLoginPhase === 'success' && isWeixinPersonalChannel(state.selected)) {
                void finalizeWeixinBindingSuccess(state.selected);
            }
        }
        ...
        function getActiveWeixinRuntimeSnapshot() {
            const loginSnapshot = state.weixinLoginSessionId
                ? window.__openclawGetCommandSessionSnapshot?.(state.weixinLoginSessionId)
                : null;
            const loginRuntimeActive = (loginSnapshot?.status === 'running')
                || isWeixinRuntimeLocallyActive('login');
            const preferChecking = !loginRuntimeActive && (
        ...
        async function finalizeWeixinBindingSuccess(channelName = state.selected) {
            if (state.weixinBindingSuccessDialogShown) return;
            state.weixinBindingSuccessDialogShown = true;

            const modal = getWeixinDialogModal();
            const actionStatusEl = modal?.querySelector('[data-role="weixin-action-status"]') || null;
            const canPersistCurrentDialog = Boolean(
                modal
                && modal.dataset.channelDialogKind === 'weixin'
                && modal.querySelector('[data-field="enabled"]')
                && modal.querySelector('[data-field="agentBinding"]')
            );

            if (actionStatusEl) {
                actionStatusEl.textContent = '个人微信接入成功，正在自动保存并关闭窗口...';
            }

            try {
                if (canPersistCurrentDialog) {
                    await persistWeixinDialogConfig(channelName, modal, null, { refresh: true });
                }
                state.status = '个人微信接入成功，已自动保存并关闭窗口。';
                if (actionStatusEl) {
                    actionStatusEl.textContent = '个人微信接入成功，已自动保存，窗口即将关闭。';
                }
            } catch (error) {
                state.weixinBindingSuccessDialogShown = false;
                state.status = error?.message || String(error);
                if (actionStatusEl) {
                    actionStatusEl.textContent = `个人微信已接入成功，但自动保存失败：${state.status}`;
                }
                return;
            }

            window.setTimeout(() => {
                const activeClose = window.__openclawActiveFormDialogClose;
                if (typeof activeClose === 'function') {
                    activeClose('success');
                    return;
                }
                modal?.closest('.ocp-dialog-overlay')?.remove();
            }, 650);
        }
        ...
        function refreshWeixinDialogRuntimeUI() {
            ...
            const runtimeLoginLink = state.weixinLoginLink || extractLatestLoginLink(runtimeLogSource);
            const runtimeAsciiQr = state.weixinLoginAsciiQr || extractLatestAsciiQr(runtimeLogSource);
            const visibleLoginLink = state.weixinLoginPhase === 'checking' ? '' : runtimeLoginLink;
            const visibleAsciiQr = state.weixinLoginPhase === 'checking' ? '' : runtimeAsciiQr;
            ...
            const hasRuntimeArtifacts = Boolean(
                String(runtimeLoginLink || '').trim()
                || String(runtimeAsciiQr || '').trim()
                || runtimeLogPreview
            );
        ...
        function openWeixinBindingDialog(channelName = WEIXIN_PERSONAL_CHANNEL_KEY, options = {}) {
            ...
            const activeRuntimeSnapshot = getActiveWeixinRuntimeSnapshot();
            const shouldPreserveRuntimeState = activeRuntimeSnapshot?.status === 'running';

            if (!shouldPreserveRuntimeState) {
                resetWeixinLoginState({
                    phase: bindingComplete ? 'success' : 'idle',
                    summary: bindingComplete ? '个人微信已确认接入' : '自动安装流与手动回退流都支持在此页完成扫码。',
                    detail: bindingComplete
                        ? '当前个人微信 V1 已完成扫码接入；如需重新绑定，可再次点击“扫描接入微信”。'
                        : '推荐优先先安装/升级插件，再在当前弹窗里完成扫码登录。'
                });
            }
        ...
        function runWeixinStatusCheck(reason = 'manual') {
            ...
            const loginSnapshot = state.weixinLoginSessionId
                ? window.__openclawGetCommandSessionSnapshot?.(state.weixinLoginSessionId)
                : null;
            const priorLoginSignal = parseWeixinLoginSignal(state.weixinLoginRawLog || '');
            const acceptedLoginAlreadyDetected = hasWeixinAcceptedLoginSignal(priorLoginSignal, state);
            const keepLoginStage = reason === 'auto'
                && !acceptedLoginAlreadyDetected
                && !state.weixinLoginFinished
                && !state.weixinLoginExpired
                && (
                    (loginSnapshot?.status === 'running')
                    || isWeixinRuntimeLocallyActive('login')
                );
            const patch = {
                weixinStatusCheckSessionId: sessionId,
                weixinStatusCheckBackground: keepLoginStage,
                weixinStatusCheckRawLog: buildWeixinRuntimeLog(
                    state.weixinStatusCheckRawLog || '',
                    reason === 'auto'
            ...
            if (!keepLoginStage) {
                patch.weixinLoginStartedAt = Date.now();
                patch.weixinLoginPhase = 'checking';
                patch.weixinLoginSummary = '正在确认连接状态';
                patch.weixinLoginDetail = reason === 'auto'
        ...
        function applyWeixinLoginSignal(logText, options = {}) {
            ...
            const shouldPromoteToChecking = hasWeixinAcceptedLoginSignal(parsed, state);

            if (shouldPromoteToChecking) {
                Object.assign(patch, {
                    weixinLoginPhase: 'checking',
                    weixinLoginSummary: '已完成扫码，正在确认连接',
                    weixinLoginDetail: parsed.configOverwritten && !parsed.loginSuccess
                        ? '已检测到配置写回标志，通常说明扫码已被接受；正在追加一次状态确认。'
                        : '已检测到登录成功或网关重启标志，正在追加一次状态确认。',
        ...
        function handleWeixinStatusCheckResult(logText, code) {
            const loginSnapshot = state.weixinLoginSessionId
                ? window.__openclawGetCommandSessionSnapshot?.(state.weixinLoginSessionId)
                : null;
            const loginStillRunning = (loginSnapshot?.status === 'running')
                || isWeixinRuntimeLocallyActive('login');
            if (!loginStillRunning) {
                clearWeixinRuntimePulse();
            }
            ...
            const priorLoginSignal = parseWeixinLoginSignal(state.weixinLoginRawLog || '');
            const priorAcceptedLogin = hasWeixinAcceptedLoginSignal(priorLoginSignal, state);
            const statusOutputBlank = !String(logText || '').trim();
            const patch = {
            ...
            if ((code === 0 || code === '0') && priorAcceptedLogin && (statusOutputBlank || !parsed.mentionsWeixin)) {
                Object.assign(patch, {
                    weixinLoginPhase: 'success',
                    weixinLoginSummary: '个人微信已完成接入',
                    weixinLoginDetail: priorLoginSignal.loginSuccess
                        ? '扫码命令已经明确返回连接成功；这次状态检查没有提供更多输出，界面已先按成功完成收口。'
                        : '已检测到扫码后的配置写回或重启信号；虽然这次状态检查没有返回更多输出，但通常已表示接入完成。',
                    status: '个人微信已完成扫码接入。'
                });
                updateWeixinLoginView(patch);
                return;
            ...
            if (priorLoginSignal.hardFailure) {
                Object.assign(patch, {
                    weixinLoginPhase: 'failure',
                    weixinLoginSummary: '扫码登录失败',
                    weixinLoginDetail: hasWeixinFatalCrash(state.weixinLoginRawLog || '')
                        ? '扫码命令已经出现崩溃堆栈，后续状态检查也没有确认成功；请查看原始日志后重试。'
                        : '扫码命令已经失败，后续状态检查也没有确认成功；请查看上方日志后重试。',
                    status: '个人微信扫码登录失败，请查看日志后重试。'
                });
                updateWeixinLoginView(patch);
                return;
        ...
        function renderWeixinLoginConsoleCard() {
            ...
            const hasLink = Boolean(state.weixinLoginLink);
            const runtimeDisplay = getWeixinRuntimeDisplayState();
            const stepTimeline = getWeixinStepTimeline(state);
            const runtimeLogSource = getCombinedWeixinRuntimeLog(runtimeDisplay.snapshot?.logText || '');
            ...
            return `
                <div class="ocp-card" style="margin-bottom:16px">
                    <div class="ocp-card-title">扫码进展 · 个人微信 V1</div>
                    ...
                    <div class="ocp-channel-step-timeline">
                        ${stepTimeline.map((item, index) => `
                            <div class="ocp-channel-step-item is-${esc(item.state)}">
                                <div class="ocp-channel-step-marker">
                                    <span class="ocp-channel-step-dot">${index + 1}</span>
                                    ${index < stepTimeline.length - 1 ? '<span class="ocp-channel-step-line"></span>' : ''}
                                </div>
                                <div class="ocp-channel-step-copy">
                                    <div class="ocp-channel-step-title">${esc(item.title)}</div>
                                    <div class="ocp-channel-step-detail">${esc(item.detail)}</div>
                                </div>
                            </div>
                        `).join('')}
                    </div>
        ...
        function handleWeixinCommandObserver(event) {
            ...
            if (event.id === state.weixinLoginSessionId) {
                ...
                if (event.type === 'finished') {
                    clearWeixinRuntimePulse();
                    state.weixinLoginFinished = true;
                    const finalSignal = parseWeixinLoginSignal(logText || state.weixinLoginRawLog || '');
                    const acceptedFinalSignal = hasWeixinAcceptedLoginSignal(finalSignal, state);
                    if ((event.code === 0 || event.code === '0') && acceptedFinalSignal) {
                        updateWeixinLoginView({
                            weixinLoginPhase: 'checking',
                            weixinLoginSummary: '已识别到扫码成功，正在确认连接',
                            weixinLoginDetail: finalSignal.loginSuccess
                                ? '扫码命令已经返回连接成功，系统正在补做一次状态确认。'
                                : '命令结束前已检测到配置写回或重启信号，系统正在补做一次状态确认。',
                            status: '已识别到个人微信扫码成功，正在自动确认连接状态。'
                        });
                        scheduleWeixinStatusCheck(600, 'auto');
                    } else if (!(event.code === 0 || event.code === '0') && state.weixinLoginPhase !== 'expired') {
                        updateWeixinLoginView({
                            weixinLoginPhase: 'failure',
            ...
            if (event.id === state.weixinStatusCheckSessionId) {
                if (event.type === 'finished') {
                    ...
                    if (!state.weixinStatusCheckBackground) {
                        patch.weixinLoginPhase = 'checking';
                        patch.weixinLoginSummary = '正在确认连接状态';
                        patch.weixinLoginDetail = '状态检查命令已发出，正在等待明确结果。';
                    }
                    updateWeixinLoginView(patch);
                    if (!state.weixinStatusCheckBackground) {
                        scheduleWeixinRuntimePulse('checking');
                    ...
                    if (!state.weixinStatusCheckBackground) {
                        patch.weixinLoginPhase = 'checking';
                        patch.weixinLoginSummary = '正在确认连接状态';
                        patch.weixinLoginDetail = '状态检查命令已启动，正在等待明确结果。';
                    }
                    updateWeixinLoginView(patch);
                }
                return;
        ...
        async function ensureChannelInstalledBeforeSave(channelName, dialog) {
            if (isWeixinPersonalChannel(channelName)) {
                const pluginStatus = ensureObject(state.weixinPluginStatus)?.installed
                    ? state.weixinPluginStatus
        ...
        const render = () => {
            ...
            const selectedProfile = getChannelProfile(state.selected, ensureObject(state.config?.channels?.[state.selected]));
            const showInstallLog = hasChannelInstallLog(state.selected);
            const showWeixinConsole = shouldShowWeixinLoginConsole();
            const channelGroups = buildConfiguredChannelGroups(state, channelKeys);
            const groupsByProfile = channelGroups.reduce((acc, group) => {
                acc[normalizeChannelKey(group.profileKey)] = group;
                return acc;
            }, {});
            ...
            container.innerHTML = `
                <div class="ocp-shell channels-compact-shell">
                    <div class="ocp-card channels-page-shell">
                        <div class="channels-page-toolbar">
                            <div class="tab-bar" id="channels-page-tabs">
                                <button type="button" class="tab ${state.activeTab === 'channels' ? 'active' : ''}" data-ch-tab="channels">渠道接入</button>
                                <button type="button" class="tab ${state.activeTab === 'agents' ? 'active' : ''}" data-ch-tab="agents">Agent 绑定</button>
                            </div>
                            <div class="channels-page-actions">
                                <button class="ocp-btn primary" id="channelsAddBtn">新建渠道</button>
                                <button class="ocp-btn" id="channelsReloadBtn">刷新</button>
                            </div>
                        </div>
                        <div class="channels-tab-panel ${state.activeTab === 'channels' ? '' : 'is-hidden'}">
                            <div class="channels-inline-status">
                            ...
                            ${showWeixinConsole ? renderWeixinLoginConsoleCard() : (showInstallLog ? `
                                <div class="ocp-card channels-log-card">
                                    <div class="ocp-card-title">安装进展${state.selected ? ` · ${esc(selectedProfile.label || state.selected)}` : ''}</div>
                                    <div class="ocp-row-meta" id="channelInstallLogMeta">${esc(buildChannelInstallLogMeta(state.selected))}</div>
                                    <pre id="channelInstallLog" class="channels-install-log">${esc(String(ensureObject(state.installLogs[normalizeChannelKey(state.selected)])?.text || '').trim())}</pre>
                                </div>
                            ` : '')}
                            ${channelGroups.length ? `
                                <div class="config-section">
                                    <div class="config-section-title">已接入渠道</div>
                                    <div class="ocp-row-meta">已写入配置的聊天工具会在这里集中展示，按钮层级对齐参考项目。</div>
                                    <div class="platforms-grid configured-grid">
                                        ${channelGroups.map((group) => renderConfiguredChannelGroupCard(group, state)).join('')}
                                    </div>
                                </div>
                            ` : ''}
                            <div class="config-section">
                                <div class="config-section-title">可接入聊天工具</div>
                                <div class="ocp-row-meta">这里保留全部模板按钮，已经接入的工具也会显示“已接入”状态，点击后直接编辑。</div>
                                <div class="platforms-grid available-grid">
                                    ${availableKeys.map((channelName) => renderAvailableChannelPick(channelName, state, groupsByProfile)).join('')}
                                </div>
                            </div>
                        </div>
                        <div class="channels-tab-panel ${state.activeTab === 'agents' ? '' : 'is-hidden'}">
                            <div class="config-section">
                                <div class="config-section-title">Agent 渠道路由</div>
                                <div class="ocp-row-meta">当前按 Agent 汇总通信渠道绑定关系，方便直接核对每个 Agent 正在接收哪些聊天工具流量。</div>
                                <div class="agent-bindings-root">
                                    ${renderChannelAgentBindingsPanel(state, channelGroups)}
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            container.querySelectorAll('[data-ch-tab]').forEach((button) => {
                button.onclick = () => {
                    state.activeTab = button.getAttribute('data-ch-tab') || 'channels';
                    render();
                };
            });
            ...
            container.querySelectorAll('[data-channel-create-preset]').forEach(button => {
                button.onclick = (event) => {
                    event?.stopPropagation?.();
                    const presetKey = button.getAttribute('data-channel-create-preset') || '';
                    state.selected = presetKey || state.selected;
                    state.pairing = [];
                    state.pairingChannel = '';
                    loadPairingFromCache(state.selected);
                    if (!openChannelCreateDialog(presetKey)) {
                        render();
                    }
                };
            });
            ...
            container.querySelector('#channelsAddBtn').onclick = async () => {
                ...
                const selectedPreset = state.selected
                    ? inferChannelProfileKey(state.selected, selectedConfig)
                    : '';
                openChannelCreateDialog(selectedPreset || presetKeys()[0] || 'qqbot');
            };
    ...
    function jobToForm(job) {
        ...
        return {
            ...
            model: String(payload?.model || job?.raw?.model || ''),
            thinking: String(payload?.thinking || job?.raw?.thinking || 'medium'),
            channel: String(job?.delivery?.channel || ''),
            to: String(job?.delivery?.to || ''),
            sessionTarget: String(job?.sessionTarget || (payload?.systemEvent || job?.raw?.systemEvent ? 'main' : 'isolated')),
            systemEvent: String(payload?.systemEvent || job?.raw?.systemEvent || ''),
    ...
    async function renderCronPage(container) {
        ...
        const state = container.__openclawCronPageState || {
            ...
            refreshPromise: null,
            pendingJobActions: {},
            cronFormLookups: null,
            cronFormLookupsAt: 0,
            cronFormLookupsPromise: null
        };
        ...
        const renderLoading = () => {
            ...
            container.innerHTML = `<div class="ocp-shell">${renderHeader('定时任务', '通过 OpenClaw Gateway 实时管理定时任务，异常时才回退到 CLI。')}<div class="ocp-card">加载中...</div></div>`;
        };

        container.__openclawCleanupCronPage = cleanupCronPageLifecycle;

        const withCurrentSelectOption = (options = [], currentValue = '', labelBuilder = null) => {
            ...
            if (normalized && !seen.has(`value:${normalized}`)) {
                next.unshift({
                    value: normalized,
                    label: typeof labelBuilder === 'function'
                        ? labelBuilder(normalized)
                        : `${normalized}（当前）`
                });
            }
        ...
        const buildCronModelOptions = (cfg = {}, currentValue = '') => {
            const options = [{ value: '', label: '使用默认模型' }];
            Object.entries(ensureObject(cfg?.models?.providers)).forEach(([providerKey, provider]) => {
                ensureArray(provider?.models).forEach((model) => {
            ...
            return withCurrentSelectOption(options, currentValue);
        };

        const buildCronChannelOptions = (cfg = {}, currentValue = '') => {
            const options = [{ value: '', label: '不指定渠道' }];
            Object.entries(ensureObject(cfg?.channels)).forEach(([channelName, channelConfig]) => {
                options.push({
                value: String(channelName || '').trim(),
                label: `${channelName}${channelConfig?.enabled === false ? '（未启用）' : '（已配置）'}`
                });
            });
            return withCurrentSelectOption(options, currentValue);
        };

        const buildCronTargetOptions = (cfg = {}, currentValue = '', selectedChannel = '') => {
            const candidateKeys = [
                'to',
                'target',
                'targets',
            ...
            const normalizedChannel = String(selectedChannel || '').trim();
            if (!normalizedChannel) {
                return withCurrentSelectOption(
                    [{ value: '', label: '请先选择渠道' }],
                    currentValue,
                    (value) => `${value}（当前，需补充渠道）`
                );
            }
            const options = [{ value: '', label: '不指定接收对象' }];
            const channels = Object.entries(ensureObject(cfg?.channels));

            const collectFromChannel = ([channelName, channelConfig]) => {
                const source = ensureObject(channelConfig);
            ...
            channels
                .filter(([channelName]) => !normalizedChannel || channelName === normalizedChannel)
                .forEach(collectFromChannel);

            if (!normalizedChannel && options.length <= 1) {
                channels.forEach(collectFromChannel);
            }

            return withCurrentSelectOption(options, currentValue);
        };

        const getPendingJobAction = (jobId = '') => String(state.pendingJobActions?.[String(jobId || '').trim()] || '').trim();
        const setPendingJobAction = (jobId, action) => {
            const normalizedJobId = String(jobId || '').trim();
            if (!normalizedJobId) return;
            state.pendingJobActions = {
                ...ensureObject(state.pendingJobActions),
                [normalizedJobId]: String(action || '').trim()
            };
        };
        const clearPendingJobAction = (jobId) => {
            const normalizedJobId = String(jobId || '').trim();
            if (!normalizedJobId) return;
            const nextActions = { ...ensureObject(state.pendingJobActions) };
            delete nextActions[normalizedJobId];
            state.pendingJobActions = nextActions;
        };
        const loadCronFormLookups = async ({ force = false } = {}) => {
            const cacheTtlMs = 30000;
            const cacheIsFresh = state.cronFormLookups
                && (Date.now() - Number(state.cronFormLookupsAt || 0)) < cacheTtlMs;
            if (!force && cacheIsFresh) {
                return state.cronFormLookups;
            }
            if (!force && state.cronFormLookupsPromise) {
                return state.cronFormLookupsPromise;
            }

            state.cronFormLookupsPromise = (async () => {
                let cfg = {};
                try {
                    cfg = await window.api.getOpenClawConfig() || {};
                } catch (_) {}
                ...
                const nextLookups = {
                    config: cfg,
                    agents: ensureArray(agents)
                };
                state.cronFormLookups = nextLookups;
                state.cronFormLookupsAt = Date.now();
                return nextLookups;
            })();

            try {
                return await state.cronFormLookupsPromise;
            } finally {
                state.cronFormLookupsPromise = null;
            }
        };

        async function buildCronFormLookups(form = {}) {
            const lookups = await loadCronFormLookups();
            const cfg = ensureObject(lookups?.config);
            const agents = ensureArray(lookups?.agents).length ? ensureArray(lookups.agents) : ['main'];

            return {
                config: cfg,
                agentOptions: withCurrentSelectOption(
                    ensureArray(agents).map((agentId) => ({
                        value: String(agentId || '').trim(),
                        label: String(agentId || '').trim()
                    })),
                    form.agentId || 'main'
                ),
                modelOptions: buildCronModelOptions(cfg, form.model),
                channelOptions: buildCronChannelOptions(cfg, form.channel),
                targetOptions: buildCronTargetOptions(cfg, form.to, form.channel)
            };
        }

        const parseEveryTextToMs = (value = '') => {
            const text = String(value || '').trim();
        ...
        const buildOptimisticCronJob = (payload = {}, existingJob = null, resultPayload = null) => {
            const sourceJob = ensureObject(existingJob);
            const serverJob = ensureObject(resultPayload);
            const schedule = payload.cron
                ? { kind: 'cron', expr: payload.cron }
                : payload.every
                    ? { kind: 'every', everyMs: parseEveryTextToMs(payload.every) }
                    : payload.at
                        ? { kind: 'at', at: payload.at }
                        : clone(sourceJob.schedule || {});
            const jobId = String(serverJob?.id || payload.id || sourceJob.id || `pending-${Date.now()}`);
            const rawPayload = {
                kind: payload.systemEvent ? 'systemEvent' : 'agentTurn',
                message: String(payload.message || sourceJob?.payload?.message || ''),
                model: String(payload.model || ''),
                thinking: String(payload.thinking || 'medium')
            };
            if (payload.systemEvent) rawPayload.systemEvent = String(payload.systemEvent || '');

            return {
                id: jobId,
                name: String(payload.name || sourceJob.name || jobId),
                description: String(payload.description || sourceJob.description || ''),
                enabled: payload.enabled !== false,
                agentId: String(payload.agentId || sourceJob.agentId || 'main'),
                schedule,
                payload: rawPayload,
                delivery: {
                    mode: payload.announce ? 'announce' : 'none',
                    channel: String(payload.channel || ''),
                    to: String(payload.to || '')
                },
                sessionTarget: String(payload.sessionTarget || 'isolated'),
                state: clone(sourceJob.state || {}),
                raw: {
                    ...clone(sourceJob.raw || {}),
                    ...clone(serverJob || {}),
                    payload: rawPayload,
        ...
        const upsertCronJobState = (job) => {
            const normalizedJob = ensureObject(job);
            if (!normalizedJob.id) return;
            const nextJobs = ensureArray(state.jobs).slice();
            const existingIndex = nextJobs.findIndex((item) => item.id === normalizedJob.id);
            if (existingIndex >= 0) {
                nextJobs[existingIndex] = normalizedJob;
            } else {
                nextJobs.unshift(normalizedJob);
            }
            state.jobs = nextJobs;
            saveCronViewCache(state.jobs);
        };
        const removeCronJobState = (jobId) => {
            const normalizedJobId = String(jobId || '').trim();
            const existingIndex = ensureArray(state.jobs).findIndex((item) => item.id === normalizedJobId);
            if (existingIndex < 0) {
                return { removedJob: null, removedIndex: -1 };
            }
            const nextJobs = ensureArray(state.jobs).slice();
            const [removedJob] = nextJobs.splice(existingIndex, 1);
            state.jobs = nextJobs;
            saveCronViewCache(state.jobs);
            return { removedJob, removedIndex: existingIndex };
        };
        const restoreCronJobState = (job, index = -1) => {
            const normalizedJob = ensureObject(job);
            if (!normalizedJob.id) return;
            const nextJobs = ensureArray(state.jobs).slice();
            const existingIndex = nextJobs.findIndex((item) => item.id === normalizedJob.id);
            if (existingIndex >= 0) {
                nextJobs[existingIndex] = normalizedJob;
            } else if (index >= 0 && index <= nextJobs.length) {
                nextJobs.splice(index, 0, normalizedJob);
            } else {
                nextJobs.unshift(normalizedJob);
            }
            state.jobs = nextJobs;
        ...
        const buildCronPayload = (values = {}, existingId = '') => {
            ...
            const payload = {
                ...
                scheduleMode,
                agentId: String(values.agentId || 'main').trim() || 'main',
                ...
                sessionTarget: String(values.sessionTarget || 'isolated').trim() || 'isolated',
                systemEvent: String(values.systemEvent || '').trim(),
        ...
        const openCronJobDialog = async (job = null) => {
            if (typeof window.showFormDialog !== 'function') return false;
            const form = job ? jobToForm(job) : blankJobForm();
            const lookups = await buildCronFormLookups(form);
            window.showFormDialog({
                ...
                fields: [
                    ...
                    ...(lookups.modelOptions.length
                        ? [{
                            name: 'model',
                            label: '模型',
                            type: 'select',
                            value: form.model,
                            options: lookups.modelOptions
                        }]
                        : [{
                            name: 'model',
                            label: '模型',
                    ...
                    ...(lookups.targetOptions.length
                        ? [{
                            name: 'to',
                            label: '接收对象',
                            type: 'select',
                            value: form.to,
                            options: lookups.targetOptions,
                            disabled: !String(form.channel || '').trim(),
                            hint: String(form.channel || '').trim() ? '请选择当前渠道下的接收对象。' : '请先选择渠道，再选择接收对象。'
                        }]
                        : [{
                            name: 'to',
                            label: '接收对象',
                        ...
                        }]),
                    {
                        ...
                        label: '会话模式',
                        type: 'select',
                        value: form.sessionTarget || 'isolated',
                        options: [
                    ...
                    },
                    {
                        ...
                        placeholder: '仅主会话模式需要填写',
                        hint: '当会话模式选择“主会话”时必填，例如 cron.digest。'
                    },
                    {
                        name: 'announce',
                        label: '公告结果',
                ...
                onConfirm: async (values, dialog) => {
                    ...
                    if (payload.to && !payload.channel) {
                        dialog.setStatus('选择接收对象前必须先选择渠道');
                        dialog.setFieldError('channel', '请先选择渠道');
                        return;
                    }
                    if (payload.sessionTarget === 'main' && !payload.systemEvent) {
                        dialog.setStatus('主会话模式必须填写系统事件');
                        return;
                    }
                    dialog.setStatus(payload.id ? '正在保存任务...' : '正在创建任务...', '#8f98ab');
                    ...
                    upsertCronJobState(buildOptimisticCronJob(payload, job, result?.result));
                    state.status = payload.id ? '已更新，正在后台同步...' : '已创建，正在后台同步...';
                    state.error = '';
                    dialog.close();
                    render();
                    void refreshJobs({ background: true });
                },
                onFieldChange: async (fieldName, values, dialog) => {
                    if (fieldName !== 'channel') return;
                    const nextTargetOptions = buildCronTargetOptions(lookups.config || {}, values.to, values.channel);
                    const currentTarget = String(values.to || '').trim();
                    const optionValues = nextTargetOptions.map((option) => String(option?.value ?? '').trim());
                    const fallbackTarget = optionValues.find((value) => value) || '';
                    const hasChannel = Boolean(String(values.channel || '').trim());
                    const selectedTarget = hasChannel && optionValues.includes(currentTarget) ? currentTarget : (hasChannel ? fallbackTarget : '');
                    dialog.setFieldOptions('to', nextTargetOptions, selectedTarget);
                    dialog.setFieldDisabled('to', !hasChannel);
                    if (!hasChannel) {
                        dialog.setFieldValue('to', '');
                    }
                }
            });
            return true;
        ...
        const refreshJobs = async ({ background = state.jobs.length > 0 } = {}) => {
            ...
            state.refreshPromise = (async () => {
                try {
                    ...
                    state.status = result?.source === 'gateway-rpc'
                        ? '已从网关实时刷新'
                        : result?.source === 'gateway-cli-fallback'
                            ? '网关快速通道异常，已回退到 CLI'
                            : result?.source === 'local-store'
                        ? '已从本地任务文件刷新'
        ...
        const render = () => {
            ...
            const listHint = state.loading
                ? '任务列表正在加载...'
                : '列表默认优先直连网关获取实时状态；只有异常时才会回退到 CLI 或本地缓存。';
            const listBody = state.loading && !state.jobs.length
                ? '<div class="ocp-empty">正在拉取定时任务列表...</div>'
                : state.jobs.length
                    ? state.jobs.map(job => {
                        const pendingAction = getPendingJobAction(job.id);
                        const isBusy = Boolean(pendingAction);
                        const toggleLabel = pendingAction === 'toggle'
                            ? (job.enabled !== false ? '停用中...' : '启用中...')
                            : (job.enabled !== false ? '停用' : '启用');
                        const runLabel = pendingAction === 'run' ? '执行中...' : '立即执行';
                        const deleteLabel = pendingAction === 'delete' ? '删除中...' : '删除';
                        return `
                                    <div class="ocp-job-card cron-job-card">
                                        ...
                                        <div class="ocp-toolbar">
                                            <button class="ocp-btn sm" data-cron-edit="${esc(job.id)}"${isBusy ? ' disabled' : ''}>编辑</button>
                                            <button class="ocp-btn sm" data-cron-toggle="${esc(job.id)}"${isBusy ? ' disabled' : ''}>${toggleLabel}</button>
                                            <button class="ocp-btn sm" data-cron-run="${esc(job.id)}"${isBusy ? ' disabled' : ''}>${runLabel}</button>
                                            <button class="ocp-btn sm danger" data-cron-delete="${esc(job.id)}"${isBusy ? ' disabled' : ''}>${deleteLabel}</button>
                                        </div>
                                    </div>
                                `;
                    }).join('')
            ...
            container.innerHTML = `
                <div class="ocp-shell">
                    ${renderHeader('定时任务', '通过 OpenClaw Gateway 实时管理定时任务，异常时才回退到 CLI。')}
                    <div class="ocp-toolbar">
            ...
            container.querySelector('#cronNewBtn').onclick = async () => {
                if (!await openCronJobDialog()) {
                    statusEl.className = statusClass('danger');
            ...
            container.querySelectorAll('[data-cron-edit]').forEach(button => {
                button.onclick = async () => {
                    ...
                    if (!await openCronJobDialog(job)) {
                        statusEl.className = statusClass('danger');
                        statusEl.textContent = '当前环境不支持弹窗表单';
                    }
                };
            ...
            container.querySelectorAll('[data-cron-toggle]').forEach(button => {
                button.onclick = async () => {
                    ...
                    setPendingJobAction(job.id, 'toggle');
                    state.status = job.enabled !== false ? `正在停用 ${job.id}...` : `正在启用 ${job.id}...`;
                    state.error = '';
                    render();
                    const result = await window.api.toggleCronJob({ id: job.id, enabled: job.enabled === false });
                    if (!result?.ok) {
                        clearPendingJobAction(job.id);
                        state.error = result?.error || 'Toggle failed';
                        state.status = '';
                        render();
                        return;
                    }
                    ...
                    state.status = `${job.id} 已更新，正在后台同步...`;
                    state.error = '';
                    render();
            ...
            container.querySelectorAll('[data-cron-run]').forEach(button => {
                button.onclick = async () => {
                    ...
                    setPendingJobAction(jobId, 'run');
                    state.status = `正在执行 ${jobId}...`;
                    state.error = '';
                    render();
                    const result = await window.api.runCronJob({ id: jobId });
                    if (!result?.ok) {
                        clearPendingJobAction(jobId);
                        state.error = result?.error || 'Run failed';
                        state.status = '';
                        render();
                        return;
                    }
                    clearPendingJobAction(jobId);
                    state.status = `${jobId} 已触发，正在后台同步...`;
                    state.error = '';
                    render();
                    void refreshJobs({ background: true });
                };
            ...
            container.querySelectorAll('[data-cron-delete]').forEach(button => {
                button.onclick = async () => {
                    ...
                    if (!yes) return;
                    setPendingJobAction(jobId, 'delete');
                    state.status = `正在删除 ${jobId}...`;
                    state.error = '';
                    const { removedJob, removedIndex } = removeCronJobState(jobId);
                    render();
                    const result = await window.api.removeCronJob({ id: jobId });
                    if (!result?.ok) {
                        clearPendingJobAction(jobId);
                        restoreCronJobState(removedJob, removedIndex);
                        state.error = result?.error || 'Delete failed';
                        state.status = '';
                        render();
                        return;
                    }
                    clearPendingJobAction(jobId);
                    state.status = `${jobId} 已删除，正在后台同步...`;
                    state.error = '';
                    render();
                    void refreshJobs({ background: state.jobs.length > 0 });
                };
